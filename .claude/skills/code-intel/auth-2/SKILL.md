---
name: auth-2
description: "Covers the **auth** subsystem of code-intel-platform. 19 symbols across 5 files. Key symbols: `json`, `on`, `setHeader`. Internal call density: 0 calls/symbol. Participates in 2 execution flow(s)."
---

# auth

> **19 symbols** | **5 files** | path: `code-intel/core/tests/unit/auth/` | call density: 0/sym

## When to Use

Load this skill when:
- The task involves code in `code-intel/core/tests/unit/auth/`
...
| File | Symbols | Notes |
|------|---------|-------|
| `code-intel/core/tests/unit/auth/auth-coverage.test.ts` | `makeReq`, `makeRes`, `status`, `json` +(3) | internal |
| `code-intel/core/tests/unit/auth/middleware.test.ts` | `makeReq`, `makeRes`, `status`, `json` +(2) | internal |
| `code-intel/core/tests/unit/auth/oidc.test.ts` | `tempDbPath`, `setOIDCEnv`, `clearOIDCEnv` | internal |
| `code-intel/core/tests/unit/auth/keychain.test.ts` | `setupSecretStore`, `cleanup` | internal |
| `code-intel/core/tests/unit/auth/auth.test.ts` | `tempDbPath` | internal |
...
| `on` | method | 16 | 0 | `auth/auth-coverage.test.ts` |
