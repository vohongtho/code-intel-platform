...
- **Cross-OS Agent Detection & MCP Registration** _(v1.0.3)_ — `init` and `setup` agent detection and MCP config writing now work correctly on Ubuntu, macOS, and Windows; `commandExists()` uses `execFileSync('which'/'where')` instead of a broken shell-composed string that created a stray `nul` file on Linux/macOS; OS-aware path helpers resolve the right config directories (`%APPDATA%` / `%LOCALAPPDATA%` on Windows, `~/.config` elsewhere); Amp, Claude Code, and Zed config paths corrected per platform; atomic rename retries up to 5× on Windows transient file-lock errors; `npx`-based MCP entry used for all agents to avoid `.cmd` shim resolution failures on Windows
- **Safe MCP config merging — zero data loss** _(v1.0.3-patch)_ — `mergeJsonFile()` now creates a `.bak` backup before every write; Amp uses root-level flat-key merge (`"amp.mcpServers"`) so no other `"amp.*"` settings are touched; all agents check idempotency before writing (skips if `code-intel` already registered); `readJsonSafe()` and `mcpAlreadyPresent()` helpers added; Kiro IDE (`~/.kiro/settings/mcp.json`) and OpenCode (`~/.config/opencode/opencode.json` with `"mcp"` key + array command) added as first-class `setup` agents
