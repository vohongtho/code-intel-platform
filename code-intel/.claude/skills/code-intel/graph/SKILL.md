---
name: graph
description: "Covers the **graph** subsystem of code-intel. 19 symbols across 2 files. Key symbols: `size`. Internal call density: 0.2 calls/symbol. Participates in 3 execution flow(s)."
---

# graph

> **19 symbols** | **2 files** | path: `core/src/graph/` | call density: 0.2/sym

## When to Use

Load this skill when:
- The task involves code in `core/src/graph/`
...
| File | Symbols | Notes |
|------|---------|-------|
| `core/src/graph/knowledge-graph.ts` | `KnowledgeGraph`, `createKnowledgeGraph`, `indexEdge`, `unindexEdge` +(13) | 17 exported |
| `core/src/graph/id-generator.ts` | `generateNodeId`, `generateEdgeId` | 2 exported |

## Entry Points

Start exploration here — exported symbols with no external callers:

- **`size`** `(method)` → `core/src/graph/knowledge-graph.ts:142`
...
| `getEdge` | method | 3 | 1 | `graph/knowledge-graph.ts` |
