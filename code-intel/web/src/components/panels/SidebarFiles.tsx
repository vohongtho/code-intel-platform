...
function TreeNodeView({ node, depth, onFileClick, selectedPath }: TreeNodeViewProps) {
  ...
  // Auto-expand root, first level, and single-child folders
  ...
  return (
    <div>
      {/* Directory row */}
      <div
        className="flex items-center gap-1 py-0.5 pr-2 cursor-pointer hover:bg-gray-800/50 rounded-sm group transition select-none"
        style={{ paddingLeft: `${indent + 6}px` }}
        onClick={() => setExpanded((v) => !v)}
      >
        <span className="text-gray-500 text-[10px] w-3 flex-shrink-0 text-center">
          {(sortedDirs.length > 0 || sortedFiles.length > 0) ? (expanded ? '▾' : '▸') : ' '}
        </span>
        <span className="text-[13px] mr-1">{expanded ? '📂' : '📁'}</span>
        <span className="text-xs text-gray-200 font-medium truncate flex-1 group-hover:text-white">
          {node.name}
        </span>
        <span className="text-[10px] text-gray-600 font-mono ml-1 flex-shrink-0">{totalFiles}</span>
      </div>
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-2 border-b border-gray-800/50">
        <input
          ...
          className="w-full bg-gray-900/60 border border-gray-700/50 rounded-md px-2.5 py-1.5 text-xs text-gray-200 placeholder-gray-600 focus:border-cyan-600 focus:outline-none"
        />
      </div>
      ...
      <div className="flex-1 overflow-y-auto py-1 text-sm">
        {filteredPaths.length === 0 ? (
          <p className="text-center text-gray-600 text-xs mt-4">No files match</p>
        ) : (
      ...
      {/* Count */}
      <div className="px-3 py-1.5 border-t border-gray-800/50 text-[10px] text-gray-600 font-mono">
