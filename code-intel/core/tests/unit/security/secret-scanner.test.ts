...
import type { SecretFinding } from '../../../src/security/secret-scanner.js';
...
describe('SecretScanner', () => {
  it('flags node with API_KEY = "sk-abc123..."', () => {
    ...
    graph.addNode({
      ...
      metadata: { value: 'sk-abcdefghij1234567890xyz' },
    });
  ...
  it('flags AWS access key pattern', () => {
    ...
    graph.addNode({
      ...
      name: 'AWS_ACCESS_KEY',
      filePath: 'src/aws.ts',
      metadata: { value: 'AKIAIOSFODNN7EXAMPLE' },
    });
  ...
  it('tags node metadata when secret is found', () => {
    ...
    const security = (node?.metadata?.security) as Record<string, unknown> | undefined;
