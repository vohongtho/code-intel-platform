...
- Added immutable semantic snapshot descriptors (`code-intel/core/src/snapshots/`) identifying a repository's analyzed state by Git tree plus analyzer/schema fingerprints (parser, fact schema, canonical identity, resolver, graph DDL, API-contract schema) — content/config-derived, independent of `createdAt` or machine-local paths.
- Added isolated snapshot materialization: a requested Git ref is resolved and checked out into a throwaway `git worktree` (never the caller's working tree, index, or HEAD) and analyzed via the existing pipeline (same parser/fact/identity/resolver/graph/BM25 decisions as a normal `analyze` run, no forked semantic logic), with artifacts written only to an isolated staging directory. Snapshot builds never call `publishIndexGeneration` or any other Generation V2 publication code path — proven by test, on both success and every failure path, that a repository's `current.json`, `generations/`, Git HEAD, tracked working-tree state, and the global repo registry are unchanged after a semantic graph diff.
- Added a bounded, self-healing snapshot cache (`.code-intel/snapshots/`, a sibling of — never inside — `generations/`) with max-age/max-count/max-bytes LRU eviction, per-fingerprint build coordination that never blocks on a losing race, and reopen-and-validate-before-serving semantics: a cache entry with corrupted metadata, a missing artifact, or an incompatible schema version is discarded and rebuilt rather than served.
- Added normalized node/relationship diffing keyed by canonical identity — never by display name: node adds/removes/changes by canonical node ID, relationships by (canonical source, canonical target, kind, call-site identity) so multiple call sites between the same symbols are preserved as distinct deltas, and a relationship certainty/strategy/evidence/confidence change is reported even when its endpoints are otherwise identical.
- Added conservative rename/move correlation: a symbol is only reported as `renamed`/`moved` when an unambiguous one-to-one pairing has byte-identical declaration content (a new per-node content-fingerprint sidecar computed from the real source at build time, since the persisted graph's own node-content/line-span fields turned out to be insufficient for this — see technical notes), optionally corroborated by Git's own rename detection. A shared display name alone is never sufficient; ambiguous candidates (e.g. identical-body overloads) are left as separate added/removed deltas annotated with candidate correlation metadata rather than merged.
- Added optional API-contract deltas delegating entirely to the existing `getApiDrift`/compatibility engine (no duplicated comparison rules). Flow and cluster deltas are explicitly reported as unsupported (`{ supported: false, reason }`) rather than fabricated: their current node identity is a per-analysis-run enumeration index, not a content fingerprint, and is not guaranteed stable across independent runs even when nothing changed.
...
- Added `group_contract_drift` (MCP tool, `GET /api/v1/groups/:name/drift` HTTP route, both documented in the OpenAPI spec): compares a synchronized repository group's contracts across a base/head Git ref pair or explicit per-repo snapshot IDs, using the same immutable semantic snapshots as branch-aware graph diff (no second checkout/snapshot implementation). Covers HTTP routes (delegating entirely to the existing `api_drift` compatibility engine — no duplicated rules), shared schemas (removed/newly-required properties, type-category changes, enum narrowing), and events (topic/name removal, payload-field removal/type change) where statically modeled; GraphQL and protobuf/gRPC contracts are extracted but always report `unknown` — extension points reserved, not implemented in this release. Optional `kind`/`repository_id` (stable repo ID, not the mutable registry name) inputs narrow which contracts are compared without skipping the load of any member repo's state — other repos may still be relevant consumers.
- Added stable, versioned contract identity and semantic fingerprints (`contract-identity.ts`, `contract-fingerprint.ts`): HTTP identity is service/repository identity plus normalized method/path, never route-name equality; schema/event identity is canonical declared identity, never display-name-only matching — two same-named schemas/events/routes in unrelated repositories never collide. A fingerprint captures full structural content, not just a display name/signature line: route fingerprints fold in the request/response shape-fact fingerprints already computed by the API-contract engine, and schema/event fingerprints fold in the parsed field list the comparators themselves diff against.
- Added a producer→consumer reverse index built from `group sync`'s existing cross-repo links and API-contract consumer matches, with a deterministic, configurable cap on reverse expansion — a cap hit is reported as bounded/lower-bound coverage, never as "no consumers."
- Findings are classified `compatible` / `potentially-breaking` / `breaking` / `unknown` with certainty and coverage; a producer change with no known in-scope consumer is reported as exactly that — never upgraded to "proven unused" unless the analysis scope actually establishes that stronger guarantee. A missing repository, failed snapshot, or unsupported contract kind makes coverage partial rather than silently substituting current state or collapsing to false "no impact."
- Extended `pr_impact`: when the active repository belongs to a synchronized group, `analysisMode: "semantic-snapshot"` additionally folds a `crossRepositoryContracts` section into the existing local blast radius; a failure to load group drift degrades that section's coverage rather than corrupting the local result. Existing `group_list`, `group_sync`, `group_contracts`, `group_query`, and `group_status` tools are unchanged — none require new input because drift exists.
- For an exact-certainty affected consumer on a breaking/potentially-breaking finding, results additionally surface related execution flows and `suggest_tests`-style guidance (call paths, suggested cases, existing tests) for that consumer symbol; heuristic/unknown-certainty consumers remain visible as candidates but never gain this upgrade without exact evidence.
- Incremental behavior: `group sync` now also records which contract fingerprints changed since the previous sync (`changedContractIds`) as an informational/tooling signal. Within one `group_contract_drift` call, a contract whose base and head snapshot fingerprints are provably identical (both present, non-legacy) skips the deep comparator rather than recomputing a guaranteed-empty result; any missing or legacy version data always falls back to the full comparator. There is no separate incremental link-matching engine — `group sync` always fully recomputes contracts/links/the consumer index, which is what makes this safe by construction rather than requiring reconciliation against a second, divergent fast path.
...
- Added 8 bundled workflow skills — `explore`, `debug`, `impact`, `plan`, `review`, `api-review`, `test-coverage`, `security-investigation` — each a manifest (`code-intel/core/src/agents/workflows/registry.ts`) declaring required/optional MCP tools, a Markdown asset, and per-target renderers, plus a shared evidence guide (canonical-identity-first, certainty/coverage propagation, no unproven "0 callers"/"safe" claims) inlined into every rendered workflow file.
- `code-intel analyze` now installs/updates these workflows for agent targets with a supported skill/rule mechanism — **Claude Code** (`.claude/skills/code-intel-workflows/<id>/SKILL.md`) and **Cursor** (`.cursor/rules/code-intel-workflow-<id>.mdc`) today. Other selected targets report `not-supported`, not failure; no separate `setup-workflows` command was added.
- Installed workflow files are managed via a content-fingerprint marker (not the existing `AGENTS.md`/`CLAUDE.md` block-marker mechanism, since workflow files are standalone rather than embedded blocks): rerunning `analyze` with unchanged content produces no file writes, an upstream content change updates in place, and a file edited by hand after install is left untouched and reported as a conflict rather than silently overwritten. `code-intel analyze --dry-run` now previews workflow create/update/skip/conflict/not-supported counts.
- The MCP tool inventory previously lived only as an inline, unexported array inside `createMcpServer`'s `ListToolsRequestSchema` handler; it is now hoisted to an exported `MCP_TOOL_DEFINITIONS` (`mcp-server/tool-definitions.ts`, re-exported from `mcp-server/index.ts` and the package root) so workflow manifests, workflow validation, and any future doc generation read the exact same schema the live server registers — not a second, driftable copy. A new `validateWorkflowRegistry` (`agents/workflows/validator.ts`) checks every manifest's required/optional tool names and input fields against that inventory and runs as part of `npm run build` (`agents/workflows/validate-cli.ts`, its own tsup entry — `dist/agents/workflows/validate-cli.js`), so a renamed/removed MCP tool or field fails the build instead of shipping a workflow that references it.
- Capability negotiation (`agents/workflows/capabilities.ts`) resolves a manifest's optional tools against the connected runtime's actual tool registration; an unavailable optional capability degrades to its manifest-declared fallback and named reduced guarantee (e.g. `api-review` uses `api_contract`/`api_impact`/`api_drift` when registered and otherwise falls back to `routes` + `pr_impact` + source reading, explicitly stating that response-shape compatibility wasn't proven) rather than the workflow inventing a tool call.
- Added an in-process evaluation harness (`tests/unit/agents/workflows/eval-fixtures.test.ts`, reusing the same `dispatchTool`-based fixture-seeding infrastructure as `tests/unit/mcp-server/api-contract-tools.test.ts` and `eval/run-mcp-bench.mjs`) with one fixture per workflow proving the tool signal each workflow's decision branch depends on is real: ambiguous same-name search results, a lexically-similar caller with no real call edge, a shallow `max_hops` undercounting real impact, an unsynced vs. synced cross-repo group, `pr_impact` certainty degrading independently of its risk field, a known vs. a dynamically-unresolvable API consumer, an uncovered exported symbol carrying no "not required" signal, and a scanner finding with no proven source-to-sink path. Two real prose/tool-contract mismatches were caught and fixed while building these fixtures: `blast_radius`'s `affected`/`affectedCount` always includes the target itself at depth 0 (now called out explicitly in the shared evidence guide), and `pr_impact`'s `risk` field only downgrades on genuinely incomplete evidence-store coverage — not on an edge's `certainty` alone — so `review`'s "needs verification" guidance now checks `certainty` too, not `risk` in isolation.
- Known limitation: only `claude` and `cursor` have a workflow renderer today; other agent targets that support a multi-file skill/rule mechanism can be added by extending `WorkflowTarget`/`renderParts` in `agents/workflows/installer.ts` without changing any workflow's shared reasoning content.
