...
The system SHALL retain GraphQL type kinds, fields, arguments, defaults, wrappers, and possible-type membership needed for compatibility analysis.
...
The system SHALL map GraphQL operations and selected field paths to canonical schema fields using parsed operation and schema evidence rather than display-name equality.
...
The system SHALL report protobuf/gRPC wire compatibility separately from generated-client source compatibility.
...
The system SHALL identify an RPC by canonical package, service, and method identity and require supporting binding evidence for exact provider/consumer links.
...
The system SHALL preserve `unknown` compatibility with an explicit legacy-schema boundary when persisted protocol data lacks the rich fields required by a comparator.
