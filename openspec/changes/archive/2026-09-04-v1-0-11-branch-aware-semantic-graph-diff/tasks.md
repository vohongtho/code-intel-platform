...
- [x] 12.1 **Deliberately not implemented — confirmed infeasible within this change's scope, not merely deferred.** The snapshot builder always performs a full temporary analysis; there is no seeding from a cached parent snapshot. Investigated twice: first as a documented initial-scope decision (`incremental/rollout-gate.ts`'s own comments describe dependency-aware incremental resolution as not proven in production; design.md/proposal.md call for preferring correctness over unsafe incremental shortcuts). Investigated a second time in a later session while designing a possible follow-up: traced the full execution path (`atomic-analyze.ts` → the child `analyze` action → `resolveAnalysisPlan`) and found the gap is deeper than "unproven" — `atomic-analyze.ts` serializes its `AtomicAnalysisPlan` (including a `graph: 'incremental'`/`dependencyAwareCandidate` decision) to `CODE_INTEL_ANALYSIS_PLAN`, but **nothing in the codebase reads that env var**; `incremental/artifact-delta-plan.ts`'s `buildArtifactDeltaPlan` — the function that would decide how to apply an incremental update per artifact — **has zero callers anywhere**, including its own module. `src/incremental/*` is a complete planning/delta-computation layer (invalidation closure, semantic delta, reverse-dependency index) with no executor behind it: nothing anywhere takes a computed delta and actually performs a surgical graph.db/bm25.db update instead of a full reparse. Building that executor is a large, standalone undertaking — plausibly the single biggest piece of unbuilt infrastructure in this codebase — not a wiring task, and not something to build inside this change. A design spec exploring a snapshot-scoped version of this was drafted and then withdrawn once this was discovered (`docs/superpowers/specs/2026-09-04-incremental-snapshot-seeding-design.md`, since deleted — it was built on the wrong assumption that the planning layer had a working executor behind it).
...
- [x] 12.4 Not applicable given 12.1's scope decision (no incremental build output to compare against a full build's) — confirmed, not just assumed: there is no incremental build output anywhere in the codebase to compare against.

## 13. Scale, failure and safety tests

- [x] 13.1 10k/100k-scale benchmark: `tests/performance/graph-diff-scaling.test.ts`. Exercises `diffEntitiesWithContinuity`/`diffRelationships` directly against synthetic 10k/100k-entity `NormalizedGraph` fixtures (a true end-to-end run through Git materialization + the real `analyze` child process at that scale is dominated by parser/analyzer time unrelated to the diff logic, and isn't repeatable as a fast test — see the file's header comment) — proves correctness at scale (exact removed/renamed/changed/added counts) plus a generous wall-clock ceiling and an explicit 10x-input/<30x-time ratio check that would fail on a real quadratic regression. Measured on dev hardware: 100k entities/relationships diff in ~150ms each.
- [x] 13.2 Per-phase time/memory counters: `SnapshotPhaseDurations` (`snapshots/types.ts`) adds `{ materialization, analysis, readback, fingerprinting }` to `SnapshotBuildResult.phases`, each a `PhaseMetric` (`durationMs` + `rssDeltaBytes`), populated in `snapshot-builder.ts`. `service.ts`'s `computeSemanticGraphDiff` adds a parallel `GraphDiffResponse.phases: { normalize, diff, contracts }`. `rssDeltaBytes` is the parent process's own RSS growth per phase; the `analysis` phase runs the real `analyze` pipeline in a child process (see snapshot-builder.ts's `runAnalyzeChild`) whose own memory isn't observable from the parent, so `analysis.rssDeltaBytes` does not capture it — documented as a limitation on `SnapshotPhaseDurations`, not silently omitted. `phases` is present only on a fresh build (`status: 'built'` or a build that failed partway through); a cache hit ran no phases this call and omits it — asserted in `graph-diff-safety.test.ts`.
