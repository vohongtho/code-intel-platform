...
TBD — created by archiving change `v1-0-11-graph-backed-agent-workflows`. Define bundled agent workflows that operate on top of Code Intel's graph/impact tooling, so that agents get consistent, uncertainty-preserving guidance without duplicating Code Intel's own analysis logic.
