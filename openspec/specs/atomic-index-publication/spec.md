...
Define Generation V2 atomic index publication: resolving a complete plan before touching disk, seeding staging with only the artifacts a plan requires, cloning artifacts with or without reflink support, validating a staged generation before pointer replacement, and preserving the previous published generation on any failure. This lets `code-intel analyze` avoid unnecessary staging, copying, and republication when the index is already correct, while keeping the crash-safety and rollback guarantees of immutable generation-based publication.
