...
`code-intel setup` configures global (machine/editor-level) integrations — MCP configuration, hooks, and plugins — for coding agents used with a repository. Setup determines which agents are eligible for global integration solely from the repository's saved agent selection (see `repository-agent-targets`), never by attempting every supported agent or by writing repository-scoped instruction files itself. Repository instruction files (`AGENTS.md`, `CLAUDE.md`, `.cursor/**`, `.github/**`, etc.) remain owned exclusively by `code-intel analyze` and context-writer.
