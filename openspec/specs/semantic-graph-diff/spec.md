...
Define immutable semantic snapshot identity and normalized graph comparison so `pr_impact` and related tooling can reason about meaning changes between two independently identified Git refs, rather than only mapping textual hunks onto a single graph state.
