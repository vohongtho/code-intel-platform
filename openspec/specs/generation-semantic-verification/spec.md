...
Define how a candidate generation is proven semantically compatible and reopenable before it is published, so that `current.json` never points at a generation that is structurally present but semantically collapsed, incomplete, or incompatible with the current resolver/identity/fact/evidence schemas.
