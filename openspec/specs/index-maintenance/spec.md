# Index Maintenance and Migration

## Purpose

Define safe, explicit maintenance of the Generation V2 index: configurable retention of published generations, conservative stale-staging detection and removal, a `code-intel index cleanup` command for inspecting and reclaiming old generations and staging, explicit legacy flat-artifact removal, and safe lock inspection/unlock behavior. Maintenance operations must never remove the current generation, must participate in the same repository-analysis serialization used by publication, and must run only after publication state has settled.
...
New maintenance commands MUST NOT change the default output contracts of existing commands, and MUST fail with a machine-readable, non-zero result when a maintenance action cannot safely complete.
...
Cleanup and migration MUST report per-entry classification and post-migration status so operators can verify outcomes without inspecting raw index files.
