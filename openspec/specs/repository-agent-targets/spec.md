...
`.code-intel/agent-targets.json` is the single repository-level source of truth for which coding agents a repository uses. `code-intel analyze` prompts for and persists this selection on first eligible interactive run and reuses it thereafter to generate only the selected agents' project instruction targets. `code-intel setup` (see `agent-aware-setup`) treats this file as read-only and consumes the same `selectedAgents` list to gate global integration eligibility, without maintaining a separate selection or reconfiguration path.
