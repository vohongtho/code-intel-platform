...
Define the pinned-snapshot contract that keeps every multi-artifact index read internally consistent under Generation V2. A logical operation must resolve `current.json` once, derive every graph/BM25/vector/metadata path from that one generation, and never mix artifacts from two different generations even when publication races the read. This applies to one-shot CLI commands, long-lived HTTP/MCP runtimes, and repository group operations, and it keeps published-read paths distinct from staging-write paths.
