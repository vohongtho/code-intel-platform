...
Define how the advanced program-analysis layer (universal per-function IR, CFG, dominators/post-dominators, control dependence, reaching definitions, def-use, PDG, function summaries, and bounded taint analysis) integrates with existing Code Intel workflows without a new mandatory command, without overstating certainty for unsupported or truncated constructs, and without exploding the main symbol graph by default.
