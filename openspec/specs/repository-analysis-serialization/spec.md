# Repository Analysis Serialization

## Purpose

Define per-repository exclusive locking for Generation V2 index mutation. Only one process may prepare or publish an index generation for a repository at a time: the lock must be acquired before any staging, cloning, or publication work begins; released safely only by its true owner; recovered conservatively when a lock appears stale; and correlated with staging ownership/activity metadata so cleanup never removes another live process's work. This closes the concurrency and cleanup-ownership gaps in unconditional analysis and staging cleanup.
...
A failing process MUST release only its own lock and remove only its own staging, and MUST NOT alter the active generation or another process's lock or staging.
